...
- Keep price datasets lazy-loaded and indexed. Never use prices or Cardmarket URLs from a different set when a set has no dataset. Price lookups must tolerate TCGdex `localId` formatting differences such as `59` versus dataset IDs like `059`.
- Known price mappings include `basep` -> `wizards_black_star_promos` and `sv09` -> `journey_together`; verify new set mappings against actual TCGdex set IDs, not Cardmarket names.
